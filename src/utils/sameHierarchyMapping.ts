...
export async function loadSameHierarchyMapping(
  url: string = "./data/SameHierarchialRelationships.xlsx"
): Promise<SameHierarchyMap> {
  ...
  try {
    ...
    for (let r = 1; r < aoa.length; r++) {
      ...
      for (let c = 1; c < row.length && c - 1 < colHeaders.length; c++) {
        ...
        if (!colHeader || !validLevels.has(colHeader as HierarchyLevel))
          continue;
